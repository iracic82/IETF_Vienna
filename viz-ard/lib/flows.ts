  ...
  steps: [
    ...
    },
    {
      ...
      detail: {
        ...
        rightPaneTabs: ["request"],
        sampleRequest: '// dns-aid ALREADY has capabilities from the catalog entry\'s own\n// `capabilities` field (capability_source="ard_catalog", set while\n// parsing the manifest in Step 4 — no fetch needed for that part).\n//\n// It STILL dereferences each entry\'s card for a live endpoint:\nGET https://ietf-vienna-cap-docs.s3.amazonaws.com/ip-reputation/mcp-server-card.json\n\n// Per ARD §3.4, the catalog entry\'s `url` is the fetchable card;\n// dns-aid follows the URL (or reads inline `data` if used). If the\n// card exposes real tools + a live URL, it UPGRADES the record:\n//   capability_source → "agent_card", endpoint_source → "ard_card"\n//\n// This lab\'s reference cards are metadata-only (no tools, no live\n// URL) — the upgrade never fires, so the record keeps its Step-4\n// catalog-level values:\n//   capability_source = "ard_catalog"         (from the catalog entry)\n//   endpoint_source   = "http_index_fallback" (catalog-only agent,\n//                        card had no live endpoint to promote)\n//   trust_manifest    = {identity, attestations[], provenance[], ...}\n//\n// The trust_manifest is the real win — SPIFFE identity + attestations\n// through the ARD path. Same catalog transport, same downstream docs.',
      },
    },
    {
